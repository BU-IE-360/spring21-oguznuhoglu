...
```{r}
checkresiduals(arma_1_4)
```

All assumptions on residuals seem to hold.
...
```{r}
checkresiduals(arma_2_3)
```

All assumptions on residuals seem to hold but the normality assumption is not that strong.
...
```{r}
checkresiduals(arma_4_1)
```

Assumptions are not strongly hold. Nevertheless, we cannot improve them at this point.
...
```{r}
checkresiduals(arma_5_3)
```

Assumptions seem to hold somehow.
...
```{r}
checkresiduals(arma_7_3)
```

Assumptions are not strongly hold. But we can move on.
...
```{r}
checkresiduals(arma_8_2)
```

Assumptions on residuals seem to hold somehow.
...
```{r}
checkresiduals(arma_9_3)
```

Constant variance assumption does not seem to hold but others are fine.
